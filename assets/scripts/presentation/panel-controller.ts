...
export class PanelController {
    ...
    /** 共享 UI 基础设施（底板/按钮/标签/图标，ui-kit.ts） */
    private ui: UiKit;
    private gmNode: Node;
    ...
    private makeButton(
        ...
        return this.ui.makeButton(name, text, w, h, handler, artPath, fallbackColor, fontSize, customData);
    }

    /** UI 按钮统一点击音：插入 clickEvents 首位，任何按钮按下都有确认反馈（不影响业务 handler） */
    private makeClickSoundHandler(): EventHandler {
        return this.ui.makeClickSoundHandler();
    }
    ...
    private createStartPanel() {
        ...
        // ---- 双倍工资（次要）+ 档案入口（局外解锁进度，v1.8.1）----
        const doubleBtn = this.makeButton('DoubleSalaryBtn', '📺 双倍工资', 200, 56, 'onDoubleSalaryClick',
            'ui/ui_btn_blue', new Color(46, 65, 82), 16);
        doubleBtn.parent = this.startPanel;
        doubleBtn.setPosition(-118, -146, 0);

        const profileBtn = this.makeButton('ProfileBtn', '📋 指挥官档案', 200, 56, 'onProfileClick',
            'ui/ui_btn_blue', new Color(46, 65, 82), 16);
        profileBtn.parent = this.startPanel;
        profileBtn.setPosition(118, -146, 0);
    ...
    private makeLabel(text: string, x: number, y: number, color: Color, parent: Node, size: number = 18): Label {
        return this.ui.makeLabel(text, x, y, color, parent, size);
    }
